...
import { CompanyEntity, EventEntity } from '../../../persistence/entities';

@Injectable()
export class BankAccountUpdatedCompanyPublisher extends InvoiceCompanyBankAccountUpdatedEventPublisherBase {
  /**
   ...
   *
   * Creates an instance of BankAccountUpdatedCompanyPublisher.
   * @param {ClientProxy} proxy
   * @memberof BankAccountUpdatedCompanyPublisher
   */
  constructor(
  ...
  /**
   * El método emit en BankAccountUpdatedCompanyPublisher utiliza lastValueFrom de rxjs
   ...
   * @memberof BankAccountUpdatedCompanyPublisher
   */
  emit<Result = EventEntity, Input = CompanyEntity>(
    pattern: EventEntity,
    data: Input,
  ): Promise<Result> {
