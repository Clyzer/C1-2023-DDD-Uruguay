...
@Injectable()
export class BenefitedRepository implements IRepository<BenefitedMySqlEntity> {
  constructor(
  ...
  async findById(benefitedId: string): Promise<BenefitedMySqlEntity> {
    const benefited = await this.repository.findOneBy({
      benefitedId,
      deletedAt: undefined,
    });
    if (!benefited)
      throw new BadRequestException(
        `Benefited with id: ${benefitedId} not found`,
      );
  ...
  async update(
    benefitedId: string,
    entity: BenefitedMySqlEntity,
  ): Promise<BenefitedMySqlEntity> {
    const benefited = await this.repository.findOneBy({
      benefitedId,
      deletedAt: undefined,
    });
  ...
  async delete(benefitedId: string): Promise<boolean> {
    ...
    await this.repository.save(benefited).catch(() => {
      result = false;
    });
